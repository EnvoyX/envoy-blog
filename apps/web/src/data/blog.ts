...
import { payloadSdk } from '@/server/payload-sdk';
...
// Payload CMS
export const getPayloadPostsFn = createServerFn({ method: 'GET' })
  .validator(
    z.object({
...
export const getPayloadPostBySlugFn = createServerFn({ method: 'GET' })
  .validator(
    z.object({
      slug: z.string(),
    }),
  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...
      title: post.title,
      slug: post.slug,
      content: JSON.stringify(post.content), // serialize content as JSON string
      meta: post.meta
        ? {
            title: post.meta.title,
            description: post.meta.description,
      ...
      createdAt: post.createdAt,
      updatedAt: post.updatedAt,
      populatedAuthors: post.populatedAuthors?.map((author) => ({
        id: author.id,
        name: author.name,
      })),
...
export const getPayloadPostByIDFn = createServerFn({ method: 'GET' })
  .validator(
    z.object({
      id: z.number(),
    }),
  ...
  .handler(async ({ data }) => {
    const post = await payloadSdk.findByID({
      collection: 'posts',
      id: data.id,
      depth: 1,
    });
