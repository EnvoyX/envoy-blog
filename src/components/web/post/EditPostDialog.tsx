...
import { ImageIcon, Loader2, MailboxIcon, Pencil, Plus, Trash2 } from 'lucide-react';
...
import { deleteShortPostFn, editShortPostFn } from '@/data/post';
...
export function EditPostDialog({ initialValues }: EditPostDialogProp) {
  ...
  const isEditPostDialog = useSelector(postModalStore, (state) => state.isEditPostDialog);
  useEffect(() => {
    if (!api) {
      return;
    }
  ...
  return (
    <Dialog
      open={isOpen && isEditPostDialog}
      onOpenChange={(open) => {
        postModalStore.setState((prev) => ({
          ...prev,
          isOpen: open,
          isEditPostDialog: !prev.isEditPostDialog,
        }));
      }}
    >
      <DialogTrigger asChild>
        <button className="flex items-center gap-2 p-1 cursor-pointer w-full">
        ...
        <div className="flex flex-col md:flex-row h-full max-h-[90vh]">
          <div className="w-full md:w-2/5 p-8 border-r border-zinc-800/50 bg-zinc-900/20">
            <DialogHeader className="mb-8">
              ...
              <DialogTitle className="text-2xl font-bold text-zinc-100">Edit Post</DialogTitle>
