...
function RouteComponent() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0a0a0a] text-white flex flex-col items-center justify-center p-6 md:p-12">
      ...
      <div className="relative z-10 text-center mb-16 space-y-4">
        <h1 className="text-4xl md:text-6xl font-bold tracking-tight bg-linear-to-b from-emerald-500 to-emerald-700 bg-clip-text text-transparent">
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-7xl">
        {models.map((model) => (
          <a
            key={model.id}
            href={`chat/${model.id}`}
            className={`group relative flex flex-col p-8 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm transition-all duration-300 hover:bg-white/10 ${model.color} hover:-translate-y-1`}
          >
